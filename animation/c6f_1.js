...
// plot data
function updateData () {
  plotData = rDeque.toArray();
  if (plotData.length < graphlen) {
    y0 = 0;
  } else {
    y0 = plotData.length-graphlen;
  };
  xrange = math.range(0, 2.5, 2.5/(graphlen-1))._data;
  ...
  data = [{
      ...
      x: xrange.slice(0, plotData.length),
      y: plotData.slice(y0, plotData.length),
      type: 'scatter',
      mode: 'lines',
      line: {color: 'blue'},
      connectgaps: false,
